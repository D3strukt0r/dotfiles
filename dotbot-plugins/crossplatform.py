...
# https://github.com/msdrigg/dotbot-crossplatform
...
class CrossPlatformLink(dotbot.plugins.Link, dotbot.Plugin, CrossPlatformTask):
        ...
        for destination, source in link_tuples:
            ...
            if use_glob:
                ...
                if len(glob_results) == 1 and (
                    ...
                    if force or relink:
                        _removed, delete_success = self._delete(
                    ...
                    for glob_full_item in glob_results:
                        ...
                        # where is it going
                        glob_link_target = os.path.join(destination, glob_item)
                        if create:
                            success &= self._create(glob_link_target)
                        if force or relink:
                            _removed, delete_success = self._delete(
                                glob_full_item,
                                glob_link_target,
    ...
    def _link(
        ...
        if (
            ...
            and self._link_target(link_name) != source
        ):
            self._log.warning(
                "Invalid link %s -> %s" % (link_name, self._link_target(link_name))
            )
        ...
        elif self._is_link(link_name) and self._link_target(link_name) != source:
            self._log.warning(
                "Incorrect link %s -> %s"
                % (link_name, self._link_target(link_name))
            )
